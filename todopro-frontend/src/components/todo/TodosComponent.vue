...
const { getTodos, filterTodos } = useTodoApi()
...
<template>
    ...
    <div class="w-full">
        <div v-if="route.path !== '/'" class="flex justify-center items-center space-x-2 my-4">
            <button @click="goToPreviousPage" :disabled="todosData.currentPage <= 1"
            ...
            <button @click="goToNextPage" :disabled="todosData.currentPage >= todosData.totalPages"
                class="p-1 px-3 border rounded text-sm bg-gray-200 hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">
                Next
            </button>
        </div>
        <div class="overflow-y-auto gap-2 p-2 m-auto w-full md:grid md:grid-cols-2 min-h-[25rem] max-h-[70rem]">
            <div v-for="todo in todosArray.slice(0, limit)" :key="todo.id">
